...
Solucao construcao(Corte *padrao, float alpha, int L)
{
    ...
    while(inventario->inicio != NULL)
    {
        ...
        while(cortesPossiveis->inicio != NULL && perda >= menorPeca && menorPeca != 0)
        {
            ...
            if(repeticoes > 0)
            {
                ...
                inserirNaListaExtend(temp->sequenciaDePecasGeradas, escolhido->tamanho, escolhido->posicao, repeticoes);
    ...
    inicio->barrasUtilizadas = qtdeBarras;
    //printf("\n%d barras utilizadas \n", qtdeBarras);
...
void GRASP( Corte *padrao, int repeticoes, int L){

    float alpha = 0.1;
    int j;
    Solucao solucoesPorAlpha[10][repeticoes];
	...
	for(j = 0; j<repeticoes; j++)
	{
	    ...
	    printf("%d ", solucoesPorAlpha[(int) alpha*10][j].perdaIndividual);
	}
